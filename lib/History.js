...
History.prototype._update = function(historyMethod, relativeUrl, render, state, e) {
  ...
  window.history[historyMethod](state, null, options.fullUrl)
...
function renderOptions(e, path) {
  ...
  var prevFullUrl = window.location.pathname + window.location.search
  var nextFullUrl = parseUrl(prevFullUrl)
  var query = qs.parse(nextFullUrl.query)
  var prevPath = query.$path || '/'
  query.$path = path
  nextFullUrl.search = qs.stringify(query)
  nextFullUrl = formatUrl(nextFullUrl)
